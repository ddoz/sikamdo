    ...
    <button wire:click="back()" class="bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded mt-10"><</button>
        <div class="py-10">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <form>
                    <h4><b>Form Pengiriman Bukti Tayang</b></h4>
                                    ...
                                    @if(count($form) > 0) 
                                        ...
                                        <div class="flex flex-wrap -mx-3 mb-6">
                                            <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                                                <label for="input{{$f->id}}" class="block text-gray-700 text-sm font-bold mb-2">{{$f->kolom}}:</label>
                                                <input type="{{$arr_tipe[$f->tipe]}}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="input{{$f->id}}" wire:model="{{($arr_tipe[$f->tipe]=='file')?'files':'inputs'}}.id_{{$f->id}}">
                                            </div>
                                        </div>
                                        @endforeach
                                    @endif
